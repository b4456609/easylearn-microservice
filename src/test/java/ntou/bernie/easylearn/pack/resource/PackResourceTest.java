...
public class PackResourceTest {
    ...
    public void testSyncPacks() throws Exception {
        ...
        when(packUserClient.getUserFolder(anyString(), rc)).thenReturn("[{\"id\":\"id\",\"name\":\"name\",\"pack\":[\"pack\",\"pasck\"]},{\"id\":\"ids\",\"name\":\"namse\",\"pack\":[\"pasck\",\"paack\"]}]");

        Response result = resources.client().target("/pack/sync").request().post(Entity.json(json));

        verify(packNoteClient, times(2)).syncNote(anyString(), rc);
